...
public sealed class AdvancedClinicalAIController : ControllerBase
{
    ...
    [HttpPost("soap-draft")]
    public async Task<ActionResult<SoapNoteDraft>> SoapDraft(Guid patientId, [FromBody] SoapNoteApiRequest request, CancellationToken ct) { if (string.IsNullOrWhiteSpace(request.FreeTextNote)) return ValidationProblem(new ValidationProblemDetails(new Dictionary<string, string[]> { ["freeTextNote"] = new[] { "A free-text note is required." } })); return Ok(await _service.SummarizeSoapNoteAsync(new SoapNoteRequest(patientId, request.FreeTextNote, request.VisitContext), ct)); }
    [HttpPost("triage")]
    public async Task<ActionResult<TriageAssessment>> Triage(Guid patientId, [FromBody] TriageApiRequest request, CancellationToken ct) { if (request.Symptoms is null || request.Symptoms.Count == 0) return ValidationProblem(new ValidationProblemDetails(new Dictionary<string, string[]> { ["symptoms"] = new[] { "At least one symptom is required." } })); return Ok(await _service.AssessTriageAsync(new TriageRequest(patientId, request.Symptoms, request.Vitals, request.Notes), ct)); }
    [HttpPost("radiology-report-draft")]
    public async Task<ActionResult<RadiologyReportDraft>> RadiologyDraft(Guid patientId, [FromBody] RadiologyReportApiRequest request, CancellationToken ct) { if (string.IsNullOrWhiteSpace(request.Modality) || string.IsNullOrWhiteSpace(request.Findings)) return ValidationProblem(new ValidationProblemDetails(new Dictionary<string, string[]> { ["request"] = new[] { "Modality and findings are required." } })); return Ok(await _service.GenerateRadiologyReportDraftAsync(new RadiologyReportRequest(patientId, request.Modality, request.BodyPart, request.Findings, request.Impressions), ct)); }
}
